    ...
    <PageWrapper>
      ...
      <section className="relative h-screen w-full overflow-hidden flex items-center justify-center">
        ...
        <div className="relative z-10 text-center px-4 max-w-5xl mx-auto">
          ...
          <h1 ref={heroTextRef} className="text-5xl md:text-8xl lg:text-9xl font-serif text-white leading-tight overflow-hidden">
            <span className="block overflow-hidden">
                {COMPANY_INFO.name.split(' ').slice(0, 2).map((word, i) => (
                    <span key={i} className="inline-block mr-4">{word.split('').map((char, j) => (
            ...
            <span className="block text-paradiso-gold italic font-light overflow-hidden">
                 {COMPANY_INFO.name.split(' ').slice(2).map((word, i) => (
          ...
          >
             ...
             <div className="mt-8 relative group cursor-pointer">
                <div className="absolute -inset-1 rounded-full bg-paradiso-gold opacity-20 group-hover:opacity-40 blur transition duration-500"></div>
                <button className="relative px-8 py-3 bg-transparent border border-white/20 text-white font-sans uppercase tracking-widest text-xs hover:bg-white hover:text-black transition-all duration-300">
                    Reserve a Table
                </button>
             </div>
          </motion.div>
      ...
      <section className="py-32 bg-paradiso-dark relative overflow-hidden">
         <div className="max-w-7xl mx-auto px-4 md:px-8 mb-16">
            <h2 className="text-4xl md:text-5xl font-serif text-white mb-4">Culinary Highlights</h2>
                ...
                >
                    ...
                    <div className="pr-4">
                        <div className="flex justify-between items-baseline mb-2">
                             <h3 className="text-2xl font-serif text-white group-hover:text-paradiso-gold transition-colors">{dish.name}</h3>
                             <span className="text-paradiso-gold font-sans">${dish.price}</span>
                        </div>
                        <p className="text-gray-400 text-sm leading-relaxed line-clamp-2">{dish.description}</p>
                    </div>
                </motion.div>
            ))}
         </div>
      ...
      <section className="py-32 bg-neutral-900">
         <div className="max-w-7xl mx-auto px-4 md:px-8">
            <div className="text-center mb-16">
                <span className="text-paradiso-gold text-xs tracking-widest uppercase">The Artisans</span>
                <h2 className="text-4xl md:text-5xl font-serif text-white mt-4">Meet the Team</h2>
            </div>
                    ...
                    >
                        <div className="w-full aspect-[3/4] overflow-hidden mb-6 relative grayscale group-hover:grayscale-0 transition-all duration-700">
                            {member.image ? (
                                ...
                                <div className="w-full h-full bg-neutral-800 flex items-center justify-center text-neutral-700">No Image</div>
                            )}
                        </div>
                        <h3 className="text-2xl font-serif text-white mb-1">{member.name}</h3>
                        <p className="text-paradiso-gold text-xs tracking-widest uppercase mb-4">{member.role}</p>
      ...
      {/* Wine Selection */}
      <section className="py-32 bg-paradiso-dark" style={{ backgroundImage: 'radial-gradient(circle at center, #1a1a1a 0%, #0a0a0a 100%)' }}>
          <div className="max-w-5xl mx-auto px-4 md:px-8 text-center">
             <h2 className="text-4xl md:text-5xl font-serif text-white mb-12">Curated Cellar</h2>
                ...
                {WINE_SELECTION.map((wine, index) => (
                    ...
                    >
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                <h3 className="text-xl md:text-2xl font-serif text-white group-hover:text-paradiso-gold transition-colors">{wine.name}</h3>
                                <p className="text-gray-500 text-sm mt-1">{wine.origin} • {wine.year}</p>
                            </div>
                        </div>
                        <div className="text-right">
                             <span className="block text-paradiso-gold text-lg">${wine.price}</span>
                             <span className="text-xs text-gray-600 uppercase tracking-wider">{wine.type}</span>
                        </div>
                    </motion.div>
                ))}
             </div>
      ...
      <section className="py-32 bg-neutral-900 border-t border-white/5">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
            <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
                 <div>
                    <h2 className="text-4xl md:text-5xl font-serif text-white">Global Presence</h2>
                    <p className="text-gray-400 mt-4 max-w-md">Find us in the world's most vibrant culinary capitals.</p>
                 </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {LOCATIONS.map((loc, index) => (
                    ...
                    >
                         ...
                         <div className="p-8">
                            <h3 className="text-3xl font-serif text-white mb-6">{loc.city}</h3>
                            ...
                            <div className="mt-8 pt-6 border-t border-white/5 flex justify-between items-center cursor-pointer">
                                <span className="text-xs tracking-widest uppercase text-gray-500 group-hover:text-white transition-colors">Get Directions</span>
                                <span className="text-paradiso-gold group-hover:translate-x-2 transition-transform">→</span>
                            </div>
                         </div>
                    </motion.div>
                ))}
